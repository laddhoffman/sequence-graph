...
function processNewline() {
    if (!this.inMultiLineComment && this.currentGroupStartLineNumber !== this.lineNumber) {
        if (this.inQuote) {
            this.error = `Must escape line breaks in multi-line strings. line ${this.lineNumber} char ${this.charNumber}`;
            return false;
        }
...
module.exports = class GenericParser {
    ...
    setInitialState(func) {
        this.initialStateFunc = () => {
            const ret = func.call(this.state);
            return ret;
        }
        return this;
    }
    ...
    parse(str) {
        ...
        while (n < totalLen) {
            ...
            // log('');
            // log.dense(`${JSON.stringify(c)}`)
            // log.dense(` - ${this.name} parser`);
            ret = func.call(this.state, c);
            if (ret === false || this.state.error) {
                parseFailed = true;
                break;
            }
            // log.dense(` - state after handler has run: `, this.state);
            this.state.lastChar = c;
        }
        ...
        if (!parseFailed) {
            ...
            if (func) {
                // log('');
                // log.dense(` - calling final handler`);
                ret = func.call(this.state);
                if (ret === false) {
                    // log.dense(` - final handler returned false`);
                    parseFailed = true;
                }
                // log.dense(` - ${this.name} state after final handler has run: `, this.state);
            }
        }

        // did parsing fail? if so, report it to the caller
        if (parseFailed) {
            return false;
        }
